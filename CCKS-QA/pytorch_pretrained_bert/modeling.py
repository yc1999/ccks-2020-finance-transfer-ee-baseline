...
class BertForQuestionAnswering_GAT_withIfTriggerEmbedding(BertPreTrainedModel):
    """BERT model for Question Answering (span extraction). with if_trigger_embedding
    """
    def __init__(self, config):
        super(BertForQuestionAnswering_GAT_withIfTriggerEmbedding, self).__init__(config)
        self.bert = BertModel(config)
        self.if_trigger_embedding = nn.Embedding(2, config.hidden_size // 10)

        # 图神经网络架构
        self.conv1 = GATConv(config.hidden_size + config.hidden_size // 10, config.hidden_size + config.hidden_size // 10, heads=1, dropout=config.hidden_dropout_prob)

        # TODO check with Google if it's normal there is no dropout on the token classifier of SQuAD in the TF version
        # self.dropout = nn.Dropout(config.hidden_dropout_prob)
        self.qa_outputs = nn.Linear((config.hidden_size + config.hidden_size // 10)*2, 2)

        self.apply(self.init_bert_weights)

    def forward(self, input_ids, if_trigger_ids, token_type_ids=None, attention_mask=None, start_positions=None, end_positions=None, adj_matrixs=None):
        # Part 1: BERT层
        sequence_output, _ = self.bert(input_ids, token_type_ids, attention_mask, output_all_encoded_layers=False)
        if_trigger_embedding_output = self.if_trigger_embedding(if_trigger_ids)
        sequence_output = torch.cat([sequence_output,if_trigger_embedding_output],dim=-1)
        seq_len = sequence_output.size()[1]

        # Part 2: GAT层
        l = []
        for i in range(len(adj_matrixs)):
            data = Data(x=sequence_output[i],edge_index=adj_matrixs[i].to_sparse()._indices())
            l.append(data)

        # 主要是拼接邻接矩阵
        batch_data = Batch.from_data_list(l)
        data = Data(x=batch_data.x, edge_index=batch_data.edge_index)
        prex = data.x
        # 变成undirected图
        ...
        # import ipdb; ipdb.set_trace()
        logits = self.qa_outputs(torch.cat([prex, x], dim=-1)) # shape of [batch_size*max_seq_len, 2]

        logits = logits.view(-1, seq_len, 2) # shape of [batch_size, max_seq_len, 2]
...
class TriggerExtractor(BertPreTrainedModel):
    def __init__(self, config):
        super(TriggerExtractor, self).__init__(config)
        self.bert = BertModel(config)
        out_dims = config.hidden_size
        mid_linear_dims = 128   # 中间层
    ...
    def forward(self, input_ids, token_type_ids=None, attention_mask=None, labels=None):
        ...
        # logits就是start和end的几率，shape of [batch_size, seq_len, 2]
        logits = self.activation(self.classifier(sequence_output))

        out = (logits,)

        if labels is not None:
            loss = self.criterion(logits, labels.float())   # 需要float化
            out = (loss,) + out
        return out

class ArgExtractor(BertPreTrainedModel):
    def __init__(self, config):
        super(ArgExtractor, self).__init__(config)
        self.bert = BertModel(config)
        out_dims = config.hidden_size
        mid_linear_dims = 128   # 中间层
        self.mid_linear = nn.Sequential(
    ...
    def forward(self, input_ids, token_type_ids=None, attention_mask=None, labels=None):
        ...
        if labels is not None:
            ...
            loss = self.criterion(logits * masks, labels.float())   # 需要float化
            out = (loss,) + out
        return out